...
public abstract class Algorithm {
    ...
    public static final Algorithm CONGRUENCIAL_MULTIPLICATIVO = new Algorithm(
        ...
        public static final Algorithm CONGRUENCIAL_ADITIVO = new Algorithm(
            "Algoritmo Congruencial Aditivo", "code", new String[]{"Xn", "Xn + c", "Mod(m)", "Ri"},
            //PARAMETERS
            Parameter.seedParameter(),
            Parameter.iterationsParameter(e -> Algorithm.CONGRUENCIAL_ADITIVO.generateList()),
            ...
            public RandomNumber generate() {
                ...
                // Calculate the next value
                int xn_plus_c = (xn + c) % m;
                components.put(getColumns()[1], String.valueOf(xn + c));
                
                components.put(getColumns()[2], String.valueOf(xn_plus_c));
        
                double ri = (double) xn_plus_c / (m - 1);
                components.put(getColumns()[3], String.valueOf(ri));
        
                // Update seed for next iteration
                Parameter<Integer> seedParam = (Parameter<Integer>) getParameters().get("Semilla");
                seedParam.setValue(xn_plus_c);
        ...
        public static final Algorithm PRODUCTO_MEDIO = new Algorithm(
        "Algoritmo de Producto Medio", "code", new String[]{"Xn", "Xn * Yn", "Medio", "Ri"},
        //PARAMETERS
        Parameter.seedParameter(),
        Parameter.iterationsParameter(e -> Algorithm.PRODUCTO_MEDIO.generateList()),
        new Parameter<Integer>("n", 0) {
            @Override
            public Integer validate() {
                if (getValue() >= 0) {
                    return getValue();
        ...
        public RandomNumber generate() {
            ...
            //Parameter values
            int seed = (int)getParameters().get("Semilla").getValue();
            int m = (int)getParameters().get("m").getValue();

            //Generate a new number using the product method
            long x0 = seed;
            components.put(getColumns()[0], String.valueOf(x0));

            // Here you can use a second seed or the previous output to create a new number
            long yn = x0 * (long)x0; // Example: multiplicar por sí mismo
            components.put(getColumns()[1], String.valueOf(yn));

            // Take the middle digits (you can modify this part for better middle digit extraction)
            String ynStr = String.valueOf(yn);
            String middle = ynStr.length() > 4 ? ynStr.substring(ynStr.length() / 2 - 2, ynStr.length() / 2 + 2) : ynStr;
            int medio = Integer.parseInt(middle) % m;
            components.put(getColumns()[2], String.valueOf(medio));

            double ri = (double)medio / (m - 1);
            components.put(getColumns()[3], String.valueOf(ri));

            // Update seed for next iteration
            Parameter<Integer> seedParam = (Parameter<Integer>)getParameters().get("Semilla");
            seedParam.setValue(medio);
